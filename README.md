...
The expected time complexity of Pollard's Rho algorithm for elliptic curves is \( O(\sqrt{n}) \), where \( n \) is the order of the group generated by the elliptic curve points. Given secp256k1, this translates to approximately \( O(2^{128}) \) operations in the worst-case scenario.
